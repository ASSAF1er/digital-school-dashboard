...
function AjoutFormation({ addTraining }) {
    return (
        <Popover className="">
            {({ open }) => (
                <>
                    ...
                    >
                        <Popover.Panel className="absolute left-1/2 top-[10%] -translate-x-1/2 transform -translate-y-[0%] z-10   max-w-sm  px-4 ">
                            <Formulaire addTraining={addTraining} />
                        </Popover.Panel>
                    </Transition>
                </>
            )}
...
function Formulaire({ addTraining }) {
    ...
    useEffect(() => {
        setTraining({ id: new Date(), name, duration, mincost, maxcost, trainer1, trainer2 })
    ...
    return (
        <div className=" bg-white  shadow-md rounded-md w-[450px] py-1  ">
            <div className="p-2 bg-white flex  flex-row justify-center border-light-blue border-b-2  items-center shadow-sm  ">
                <span className="text-2xl font-bold text-[#0c0c75]">Nouvelle Formation</span>
            </div>
